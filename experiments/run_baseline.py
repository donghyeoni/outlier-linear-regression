"""Experiment 1: benchmark of four hand-coded optimizers on clean synthetic
linear-regression data, compared to the closed-form solution.
...
* ``convergence_tail.json`` -- weight-error range over the last 100 epochs
  (``init=zero``, ``batch=full``), to tell convergence from oscillation
...
def main():
    ...
    for opt in ("GD", "AdaGrad", "RMSProp", "Adam"):
        ...
        tail[opt] = {"min": float(min(last)), "max": float(max(last)),
                     "final": float(last[-1])}
    with open(os.path.join(OUT_DIR, "convergence_tail.json"), "w") as f:
        json.dump({"setting": "init=zero, batch=full, last 100 epochs",
                   "weight_error": tail}, f, indent=2)
