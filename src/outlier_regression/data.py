...
def generate_linear_data(N: int = 1000, D: int = 4, noise_std: float = 0.1,
    ...
    Notes
    -----
    The NumPy RNG call order (``rand`` -> ``rand`` -> ``randn``) defines the
    dataset for a given seed; changing it changes every result.
    """
    np.random.seed(seed)
...
def generate_mixture_data(N: int = 1000, D: int = 4, p: float = 0.9,
    ...
    Notes
    -----
    Population labels are drawn one sample at a time, after ``X``, ``w1`` and
    the noise. This order defines the dataset for a given seed; changing it
