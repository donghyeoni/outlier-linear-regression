...
This module isolates that update logic so a single trainer
(``outlier_regression.train``) can drive any of them.
"""
...
def init_state(w: np.ndarray) -> dict:
    ...
    Each accumulator is an independent zero array, so the optimizers never
    share state.
    """
    return {
...
def step(optimizer_type: str, w: np.ndarray, grad: np.ndarray, state: dict,
    ...
    learning_rate : float
    beta1, beta2 : float
        Adam moment decay rates. RMSProp uses a fixed decay of 0.9 instead.
    epsilon : float
        Numerical-stability constant for AdaGrad, RMSProp and Adam.
